...
function getCandidate(tran: any[][]) {
  let words: any[] = [];
  if (tran[1]) {
    words = words.concat(tran[1][0][1]);
  }
  if (tran[5]) {
    let candidates = tran[5].map((tt) => (tt[2] || [tt[0]]).map((t: any[]) => t[0]));
...
async function translate(word: any, lang: { from: any; to: any }) {
  ...
  try {
    ...
    translate[0].pop();
    if (lang.from == 'auto') {
      lang.from = translate[2];
    }
    return {
      lang,
      word,
      text: translate[0].map((t: any[]) => t[0]).join(''),
      candidate,
    };
  } catch (err: any) {
...
export const google = Object.assign(
  async (word: any, { from = 'auto', to = 'zh' }) => {
    let lang = {
      from: from || 'auto',
      to: to || 'zh',
    };

    const input = wordSplit(word).join(' ').toLowerCase();

    return await translate(input, lang);
  },
  {
    auto: 'auto',
    sq: 'sq',
